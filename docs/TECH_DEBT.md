...
- [x] **1.4 Legacy reactivity emission path** — resolved (globals residue tracked in §1.5).
  ...
  (`emit_trigger_for_signal`). The one survivor is `LirOp::TriggerEffects`,
  now documented as the **global**-signal fanout mechanism for observers
  without synthesized inline fanout blocks (`context.rs` `global_fanout_blocks`
  gate) — it goes away when `synth_global_fanout_blocks` covers every
  observer shape (today it's gated to simple all-i32 effect params), not
  on its own.
...
- [x] **Incomplete materialize paths.** Resolved by deletion: `lower_to_lir/signals_inline.rs` (the inline *memory* signal-write helpers with their "bail until const-materialize" todos) is gone entirely — no signal or global write lowers to linear memory anymore, so there is nothing left to materialize into memory.
...
[`ARCHITECTURE.md`](ARCHITECTURE.md). When you *fix* one of these, tick its
