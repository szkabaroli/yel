...
stack / in a block" is `wasm/repr.rs::InternalRepr` (`Scalar`, `FatPointer`,
`GcRef(struct_idx)`, `GcArrayRef(arr_idx)`, `FlatGcStruct(idx)`, …). The
half-migrated state is the debt:

- **Per-type, partial:** records / tuples / typed-array lists are already single GC refs, but option/result/variant are "typed `FlatGcStruct` ref **when migrated, otherwise canonical-flat**" (`wasm/repr.rs` docstring). Both representations coexist and several emit sites still decompose types independently — the exact inconsistency `repr.rs` was created to stamp out, and it "doesn't yet wrap the full push/pop/store/load emission surface … will grow as more emit sites migrate."
...
- **Dual signal storage is a symptom:** pointer-repr signals (records/tuples) are stored in **both** the `$Comp` GC struct **and** linear memory (`SignalStorage`, memory `project_signal_storage_dual`). The `mem` backing should disappear for these once they're fully on GC; today both are kept "to preserve semantics."
