...
- **5e — delete `TreeShape`, `TreeBoundary`, `TreeBoundaryKind`, `TreeFieldDecl`, `BoundaryField` slot kind, `boundary_params`, `ChildrenArray*` ops, `tree_shape.rs`:** **deferred — multi-session work.**
  ...
  **5e-1 — drive Stage 5b's "remaining BoundaryField loads/stores" count to zero.** Today 8 (counter) / 4 (list_append) BoundaryField uses survive the rewriter, all in handler/dispatch blocks that access `$self.tree`-rooted boundaries with no in-block binding. Closing them requires either (a) a new `BoundaryRefFromSelf { boundary_id, result }` LIR op that codegen translates to the chain walk currently inside `emit_boundary_ref`, with the rewriter emitting one per unique unbound boundary id at block entry; or (b) restructuring the lowerer to emit explicit `BindBoundaryLocal` / chain-walk ops for each `$self`-rooted boundary the block needs. Option (a) is smaller: ~50 LOC for the new op + rewriter extension. Until done, `emit_boundary_ref`'s chain walk is alive code, blocking deletion of `LirSlotKind::BoundaryField`.

  **5e-2 — delete `boundary_params` field.** Has an ordering dependency with `dedupe_update_blocks`: dedupe runs at line 1769 of `lower_component`, but `allocate_boundary_param_slots` runs at line 1779. To migrate dedupe's 3 reads to `boundary_param_slots`, slots have to exist when dedupe runs — i.e. swap the pass order so allocation precedes dedupe. Risk: dedupe might collapse blocks whose slot allocations diverge. Need to verify by walking through dedupe's structural-equality check with a per-block-slot-aware comparator. Estimated: 1 focused session.
  ...
  **5e-4 — delete `LirSlotKind::BoundaryField`, `tree_shape.rs`, and the `tree_shape` field on `LirResource`.** Blocked on 5e-1, 5e-2, 5e-3, plus migrating `block_lower.rs`'s ~20 internal `tree_shape` reads. The lowerer reads `tree_shape` extensively during synthesis; migrating means rewriting `block_lower::lower_component` to consume only the registry. Stage 5e-4 is then mechanical but high-volume.
