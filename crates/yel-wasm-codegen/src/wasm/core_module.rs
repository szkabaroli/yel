...
use yel_core::{DefId, DefKind, FieldIdx, Ty};

use super::CodegenError;
use crate::wasm::AllocatorFuncs;
use yel_core::lir::{BlockId, LirComponent, LirExpr, LirOp, SlotKind, SlotValType, align_to};
...
use super::{
    ...
    IMPORT_CREATE_TEXT, IMPORT_INSERT_AFTER, IMPORT_REMOVE, IMPORT_SET_ATTRIBUTE,
    IMPORT_SET_TEXT_CONTENT, ImportLayout, MemoryLayout, NUM_DOM_IMPORTS, WasmPackageBuilder,
    to_kebab_case, to_wit_name,
};
...
impl<'a> WasmPackageBuilder<'a> {
    pub(crate) fn build_core_module(&mut self) -> Result<Module, CodegenError> {
        ...
        for &(elem_ty, count) in &self.list_constructs {
            ...
            let type_idx = match total_params {
                ...
                _ => {
                    eprintln!(
                        "WARN: No type for list ctor with {} params, using type 17",
                        total_params
                    );
                    17
                }
        ...
        for (comp_idx, component) in self.components.iter().enumerate() {
            for block in &component.blocks {
                ...
                let type_idx = if !block.captured_locals.is_empty() {
                    3
                } else {
                    1
                };
        ...
        for (_comp_idx, exported_comp) in exported_components.iter().enumerate() {
            ...
            exports.export(
                &format!("{}[method]{}.dispatch", interface_prefix, prefix),
                ExportKind::Func,
                comp_func_idx + 3,
            );
        ...
        for comp_idx in 0..self.components.len() {
            ...
            for sig_idx in 0..num_signals {
                code.function(&self.generate_getter_for(
                    &self.components[comp_idx],
                    &layout,
                    sig_idx,
                )?);
    ...
    fn generate_name_section_multi(
        &self,
        module: &mut Module,
        ...
        for (comp_idx, component) in self.components.iter().enumerate() {
            ...
            for slot in &component.slots {
                if let SlotKind::Temp { local_idx } = slot.kind {
    ...
    fn generate_constructor_for(
        ...
        let has_string_signals = component
            .signals
            .iter()
            .any(|s| matches!(self.ctx.ty_kind(s.ty), InternedTyKind::String));
        let has_list_signals = component
            .signals
            .iter()
            .any(|s| matches!(self.ctx.ty_kind(s.ty), InternedTyKind::List(_)));
        ...
        for slot in &component.slots {
            if let SlotKind::Memory { offset, size } = slot.kind {
    ...
    fn generate_mount_for(
        ...
        let max_temp_slot = component
            .slots
            .iter()
            .enumerate()
            .filter(|(_, s)| matches!(s.kind, SlotKind::Temp { .. }))
            .map(|(i, _)| i as u32)
            .max()
            .unwrap_or(0);
    ...
    fn emit_op(
        ...
        let layout = self
            ...
            .get(comp_idx)
            .cloned()
            .unwrap_or_else(|| MemoryLayout {
                base: 324,
                signal_offsets: component
                    .signals
                    .iter()
                    .enumerate()
                    .map(|(i, _)| (i as i32) * 4)
        ...
        match op {
            ...
            LirOp::StoreHandle { slot, from } => {
                if let Some(slot_info) = component.slots.get(slot.0 as usize) {
                    if let SlotKind::Memory { offset, .. } = slot_info.kind {
                ...
                for effect in &component.effects {
                    if effect.dependencies.contains(signal) {
                        if let Some(&func_idx) = self
                            .block_func_indices
                            .get(&(comp_idx, effect.update_block))
                        {
                            func.instruction(&Instruction::I32Const(0));
                            func.instruction(&Instruction::Call(func_idx));
                        }
            ...
            LirOp::LoadListFromLocal {
                ...
                let base_slot = if let Some(captured_map) = &self.current_block_captured_locals {
                    captured_map
                        .get(base_local)
                        .map(|&slot| slot + local_offset)
                } else {
                    None
                }
                .or_else(|| {
                    self.current_block_local_to_slot
                        .as_ref()
    ...
    fn generate_unmount_for(
        ...
        for slot in &component.slots {
            if let SlotKind::Memory { offset, .. } = slot.kind {
                func.instruction(&Instruction::I32Const(offset as i32));
                func.instruction(&Instruction::I32Load(mem_arg(0, 2)));
                func.instruction(&Instruction::Call(IMPORT_REMOVE));
            }
    ...
    fn generate_dispatch_for(
        ...
        let mount_block = component.get_block(component.mount_block);
        let mut handlers: Vec<(u32, BlockId)> = Vec::new();
        for op in &mount_block.ops {
            if let LirOp::AddEventListener { handler, .. } = op {
    ...
    fn generate_block_function(
        ...
        let num_slots = self
            .count_slots_in_ops(&block.ops)
            .max(if has_captures { 2 } else { 1 });
    ...
    fn count_slots_in_ops(&self, ops: &[LirOp]) -> u32 {
    ...
    fn compute_field_offset(&self, record_def: DefId, target_field_idx: FieldIdx) -> u32 {
