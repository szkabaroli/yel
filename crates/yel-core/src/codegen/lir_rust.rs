...
pub fn generate_rust(component: &LirComponent, ctx: &CompilerContext) -> String {
    let mut gen = RustCodegen::new(ctx);
    gen.generate_component(component)
}
...
mod tests {
    ...
    fn test_emit_expression() {
        ...
        assert_eq!(gen.emit_expr(&expr), "42");
        ...
        assert_eq!(gen.emit_expr(&binary), "(1 + 2)");
    }
    ...
    fn test_emit_type() {
        ...
        assert_eq!(gen.emit_type(Ty::S32), "i32");
        assert_eq!(gen.emit_type(Ty::BOOL), "bool");
        assert_eq!(gen.emit_type(Ty::STRING), "String");
    }
}
