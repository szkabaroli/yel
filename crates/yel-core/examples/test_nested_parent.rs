//! Test for-loop with nested field access (for sub in item.subitems)

use yel_core::codegen::generate_wasm;
use yel_core::Compiler;

fn main() {
    // Full nested version: iterate over a list of records that contain sub-lists
    let source = r#"
package yel:nested-parent@1.0.0;
...
"#;
    ...
    match generate_wasm(&[lir], compiler.context()) {
        Ok(wasm_bytes) => {
            std::fs::write("/tmp/test_nested_parent.wasm", &wasm_bytes).expect("Failed to write WASM");
            println!("Generated {} bytes of WASM to /tmp/test_nested_parent.wasm", wasm_bytes.len());
