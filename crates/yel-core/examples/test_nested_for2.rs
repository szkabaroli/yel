//! Test nested for-loops with outer variable access
...
fn main() {
    let source = r#"
        ...
        export component Counter {
            ...
            VStack {
                Text { "Nested for-loops test" }

                for item in items key(item.name) {
                    VStack {
                        Text { "{item.name}" }
                        
                        // Nested for-loop - needs access to outer 'item'
    ...
    match generate_wasm(&[lir], compiler.context()) {
        Ok(wasm_bytes) => {
            std::fs::write("/tmp/test_nested_for2.wasm", &wasm_bytes).expect("Failed to write WASM");
            println!("Generated {} bytes of WASM to /tmp/test_nested_for2.wasm", wasm_bytes.len());
